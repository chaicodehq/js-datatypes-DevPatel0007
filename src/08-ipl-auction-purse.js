...
export function iplAuctionSummary(team, players) {
  if (typeof team !== "object" || team === null || typeof team.name !== "string" || typeof team.purse !== "number" || team.purse <= 0) {
    return null;
  }
  ...
  const costliestPlayer = players.reduce((costliest, player) => player.price > costliest.price ? player : costliest, players[0]);
  const cheapestPlayer = players.reduce((cheapest, player) => player.price < cheapest.price ? player : cheapest, players[0]);
